import { Layout, Clipboard, Package, PenTool, Calendar, Settings, HelpCircle, FileText } from 'lucide-react';
import { NavLink } from 'react-router-dom';

const StaffSidebar = () => {
    const menuItems = [
        { name: 'Dashboard', icon: Layout, path: '/staff/dashboard' },
        { name: 'Tasks', icon: Clipboard, path: '/staff/tasks' },
        { name: 'Inventory', icon: Package, path: '/staff/inventory' },
        { name: 'Requests', icon: PenTool, path: '/staff/requests' },
        { name: 'Schedule', icon: Calendar, path: '/staff/schedule' },
        { name: 'Reports', icon: FileText, path: '/staff/reports' },
        { name: 'Settings', icon: Settings, path: '/staff/settings' },
        { name: 'Help', icon: HelpCircle, path: '/staff/help' },
    ];

    return (
        <div className="w-64 bg-white h-screen border-r border-gray-100 flex flex-col shadow-sm hidden md:flex">
            <div className="p-6 flex items-center space-x-2">
                <div className="w-8 h-8 bg-gradient-to-tr from-green-500 to-green-600 rounded-lg flex items-center justify-center text-white font-bold text-xl">
                    S
                </div>
                <div>
                    <h1 className="text-xl font-bold text-gray-800 tracking-tight">Staff</h1>
